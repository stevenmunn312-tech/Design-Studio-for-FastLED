...
- [x] **Starter templates** — `src/state/starterTemplates.ts` builds six pre-wired graphs (Rainbow Sweep, Fire, Scrolling Text, Audio Spectrum, Field Warp Demo, and a Show Pipeline skeleton covering both show workflows) straight from `NODE_LIBRARY` defaults, loaded through the existing `loadGraph`. MenuBar's **✦ Templates** button opens `TemplatesPopup` (same overlay pattern as Recover), confirming before it replaces a non-empty workspace.
