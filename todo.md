...
- [ ] **Add a live controller-capacity meter.** Help users decide whether they can add "just a few more patterns" by showing board-specific flash/SRAM headroom while composing, especially for `PatternCollection → PatternMaster → MatrixOutput` shows on small controllers like Arduino Uno Rev3. Treat the selected exact board target/FQBN (and any memory-affecting board menu options such as ESP32 partition/PSRAM choices) as part of the cache key; never show a generic "Arduino" capacity number. Add a backend compile-only endpoint that generates the same normal/show sketch used by Upload, runs the active engine without a serial port, parses the toolchain's real size report (`Sketch uses...` / `Global variables use...` for `arduino-cli`, fbuild's `Flash:` / `RAM:` lines where supported), and returns measured bytes, limits, percentages, warning/error state, and the raw target label. Debounce frontend checks after graph, Pattern Collection, Show Engine, Matrix Output, board, or engine changes; show `checking`, `measured`, `stale`, `toolchain missing`, and `won't fit` states rather than blocking editing. Surface the compact meter in the Matrix Output upload/setup area (`Arduino Uno Rev3 · flash 74% · SRAM 41%`) and, when possible, annotate Pattern Collection edits with the last measured delta after adding/removing a pattern. Keep the existing fast RAM estimator as the immediate fallback while the compile check is pending, but make the measured compile result the authority for Upload readiness and capacity warnings.
