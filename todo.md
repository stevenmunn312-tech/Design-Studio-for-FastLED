...
- [ ] **Improve Text authoring.** Horizontal/vertical alignment (`hAlign`/`vAlign`: left/center/right, top/middle/bottom), scroll direction (`scrollAxis`: horizontal/vertical — the same `scroll` property now animates a vertical crawl instead of only a horizontal marquee), and letter spacing (`letterSpacing`, baked into `textColumns()`) are done, shared by `src/state/font.ts` (`textAlignMode`), the evaluator, and codegen (`textAxisStartExpr` mirrors `textAlignedStart`/`normalizedCenterAxis`) so preview and firmware stay in lockstep; all default to the old fixed behavior so existing graphs are unaffected. Preview-safe multiline behavior and a clearer custom-font manager remain open.
