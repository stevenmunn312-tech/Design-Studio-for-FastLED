...
describe('custom displays in generative shows', () => {
  ...
  it('takes a wired Enabled as the panel gate', () => {
    const cpp = generate(
      [panel('tft'), node('toggle', 'ButtonInput', { pin: 12 })],
      [edge('toggle', 'pressed', 'tft', 'enabled')],
      { screen: document() },
    )
  ...
  it('requires prepared assets and emits their actual PROGMEM bytes', () => {
    ...
    expect(() => generate([panel('tft')], [], { screen: doc })).toThrow('has not been baked')
    const cpp = generateShowSketch([...root, panel('tft')], [...routing], groups, { displayDocuments: { screen: doc },
  ...
  it('reports missing documents and unsupported sources through both validation and generation', () => {
    expect(() => generate([panel('tft')], [], {})).toThrow('screen document is missing')
