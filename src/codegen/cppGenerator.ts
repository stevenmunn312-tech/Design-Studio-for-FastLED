  ...
  function emit(node: StudioNode): void {
    ...
    switch (type) {
      ...
      case 'SpectrumBars': {
        ...
        ln(`    float _motion = t * (0.8f + _spd * 5.5f);`)
        ...
        ln(`      float _ripple = sinf(_nx * 10.5f - _motion * (1.1f + _t * 1.8f)) * 0.08f * _strength;`)
        ln(`      float _shimmer = max(0.0f, sinf(_nx * 21.0f + _motion * (2.0f + _m * 2.5f))) * 0.06f * _t * _strength;`)
        ...
        ln(`        float _pulse = 0.72f + 0.28f * sinf(_vertical * 6.2f - _motion * (1.4f + _b * 1.6f));`)
        ln(`        float _v = min(1.0f, max(0.0f, (0.28f + _vertical * 0.72f) * _pulse));`)
        ln(`        float _pt = _nx * (0.7f + _b * 0.25f) + _vertical * (0.45f + _m * 0.35f) + _motion * 0.018f;`)
        ...
        ln(`        CRGB _peak = ColorFromPalette(${pal}, (uint8_t)((_nx + _motion * 0.03f) * 255));`)
