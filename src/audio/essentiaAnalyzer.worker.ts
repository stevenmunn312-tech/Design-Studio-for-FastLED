...
// ── Analysis pipeline (mono PCM → SongAnalysis) ────────────────────────────────
async function analyze(mono: Float32Array, sampleRate: number, durationMs: number, title: string): Promise<SongAnalysis> {
...
self.onmessage = async (e: MessageEvent<AnalyzeRequest>) => {
  ...
  try {
    const analysis = await analyze(mono, sampleRate, durationMs, title)
    self.postMessage({ id, ok: true, analysis } satisfies AnalyzeResponse)
  } catch (err) {
    self.postMessage({ id, ok: false, error: String(err) } satisfies AnalyzeResponse)
  }
}
