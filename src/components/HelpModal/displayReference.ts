...
export const DISPLAY_REFERENCE: Record<string, DisplayReferenceContent> = {
  ...
  TransportDisplay: {
    ...
    steps: [
      ...
      'For touch playback, connect Display Panel Controls → Player Controls → Music Player. Choosing a touch module alone does not create an action destination. Use Graph Health to check that the chosen layout and controls are supported by the selected build.',
    ],
  ...
  Display: {
    overview: [
      'A custom touch screen for the ST7789V 2.4-inch 240×320 module with XPT2046. Add it through Add Hardware → Displays → Screen design, or Create screen design on a Display panel, which sizes it to that panel. Other modules are unsupported by the custom editor.',
    ...
    steps: [
      'Choose the module wiring in Hardware and rotation on the graph node. Open Edit screen design, add widgets or insert a template, and set labels, bounds, values, theme, and background. Templates add a layout; you still wire its actions yourself.',
