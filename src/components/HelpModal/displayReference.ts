...
export const DISPLAY_REFERENCE: Record<string, DisplayReferenceContent> = {
  ...
  Display: {
    overview: [
      ...
      'Click Edit screen design on the panel. Design the screen, then press Run to float a live touch screen on the graph so you can drive wired properties and watch the LED preview. This preview does not send touches to the device or verify physical calibration.',
    ],
    steps: [
      ...
      'Use Set when another graph value should synchronize a control. Touch owns it while held; after release, the wired Set value takes over. Without Set, it keeps its local value. Run opens a live touch screen on the graph; finger values drive the same runtime the preview samples, and they persist until the graph or document changes them.',
