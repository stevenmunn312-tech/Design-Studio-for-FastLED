...
export const DISPLAY_REFERENCE: Record<string, DisplayReferenceContent> = {
  ...
  InfoDisplay: {
    ...
    steps: [
      ...
      'Connect the source’s Display output to Info Display. Pattern Slideshow owns the pattern selection; the OLED only reports it. Route physical browsing controls to the slideshow’s named action inputs, or through Control Map when you want one compact bundle.',
  ...
  TransportDisplay: {
    ...
    steps: [
      ...
      'For fixed music touch, wire named Touch outputs such as Play / Pause directly to matching Music Player action inputs, or send Touch Controls through Control Map when you need the compact bundle, volume/brightness, chaining, or repeat settings. Clock and Show Status are read-only. A custom screen publishes its individual widget outputs on the companion Touch node.',
  ...
  Display: {
    ...
    steps: [
      ...
      'For a level control, add a Slider and Numeric Readout, then wire the slider’s Output from the companion Touch node to the readout’s input and the intended control input. For SD playback levels, use Control Map → Music Player. Keep normalized brightness and volume sliders in the 0–1 range.',
