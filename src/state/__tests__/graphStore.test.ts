...
describe('graphStore — grouping', () => {
  ...
  it('leaves MatrixOutput and signal sources behind when grouping', () => {
    ...
    const gid = useGraphStore.getState().createGroup('Spectrum', ['mic', 'audio', 'fft', 'sp', 'out'])
    const s = useGraphStore.getState()

    expect(s.nodes.find((n) => n.id === 'mic')).toBeTruthy()   // source left behind
    expect(s.nodes.find((n) => n.id === 'audio')).toBeTruthy() // capability left behind
    ...
    // Audio now feeds the group via an exposed audio param; the group feeds out.
    expect(s.edges.some((e) => e.source === 'audio' && e.target === groupNode.id)).toBe(true)
    expect(s.edges.some((e) => e.source === groupNode.id && e.target === 'out')).toBe(true)
    expect(s.graphData[gid].nodes.some((n) => n.data.nodeType === 'GroupInput')).toBe(true)
  })
...
describe('graphStore — loadGraph normalization', () => {
  ...
  it('does not infer the Board from the retired build-profile field', () => {
    ...
    expect(board.data.properties.profileId).toBe('esp32-generic-devkit-38pin')
    expect(board.data.properties.psramPolicy).toBe('auto')
    expect(board.data.properties.serialRoute).toBe('auto')
    expect(useGraphStore.getState().buildProfile).not.toHaveProperty('physicalBoardProfileId')
  })
