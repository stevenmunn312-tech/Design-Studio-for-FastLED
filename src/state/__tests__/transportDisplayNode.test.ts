...
describe('TransportDisplay registration', () => {
  ...
  it('takes one content input and declares no per-field ports', () => {
    const def = NODE_LIBRARY.find((entry) => entry.type === 'TransportDisplay')!
    // One content input, like the OLED beside it. The screen a user draws is
    // not a second input: it belongs to this panel, named by `displayId`.
    ...
    expect(def.defaultProperties).toMatchObject({
      partId: PLAIN, tftLayout: 'Now Playing', tftRotation: '0', enabled: true, displayId: '',
    })
  })
