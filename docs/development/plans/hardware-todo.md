...
- [x] Resolve the fbuild ESP32 deploy/serial-port failure before reporting it upstream. On `2026-09-02`, fbuild 2.5.21 successfully flashed and verified an ESP32-S3 on COM3 when constrained to 115200 baud and the helper interpreter's pinned `esptool`; `arduino-cli` is now the recommended ESP32 path while fbuild remains an explicit experimental choice because its separate no-op delay is unresolved. The deploy failure did not reproduce, so no upstream issue was prepared. See [`fbuild-workarounds.md`](../reports/fbuild-workarounds.md#esp32-deploy-path-experiment-2026-09-02).
