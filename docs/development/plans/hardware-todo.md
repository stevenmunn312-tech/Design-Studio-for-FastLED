...
- [ ] Report that fbuild can silently ignore a pinned `platform = espressif32@...` version. Still true on 2.5.21, and a bare `platform_packages` version pin is dropped the same way; the URL forms are honoured and are how to pin an ESP32 core. See [`fbuild-workarounds.md`](../reports/fbuild-workarounds.md#upgrade-record-from-254).
- [ ] Report the reproducible fbuild deploy/serial-port failure where shell `esptool` opens the same port successfully; use `arduino-cli` as the current workaround. Nothing upstream touches this path through 2.5.21. Eliminate fbuild's high per-board deploy baud and its bare-name `esptool` spawn first — neither has been tested.
