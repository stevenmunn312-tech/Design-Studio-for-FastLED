...
**Group-input modulation (slice 2, `energy` role).** The `PerformanceGenerator.useGroupInputs` toggle drives each collected pattern's exposed `GroupInput` "roles" from the analysis. The generator emits a `SET_ENERGY` event (0–1 section energy) per section; a pattern opts in by exposing a `GroupInput` whose `paramId` is `energy` (rename it from the default `param0` in the node's property editor). **Preview:** `renderShowFrame(show, ms, W, H, groups, useGroupInputs)` evaluates the pattern group's subgraph directly (its `GroupOutput` terminal) and, when on, passes `{ energy }` through the evaluator's `groupInputs` path. **Codegen:** with `useGroupInputs`, `buildPattern` keeps role-tagged `GroupInput` nodes (instead of stripping them) and `generateCpp` emits them as `float n_<id>_out = <paramId>;`; `buildPatternRenderers(ids, groups, roleParams)` widens each `render_pN` to `(uint32_t ms, float energy)` and reports `params`; `generatePlayerSketch` declares the `energy` global, handles `CMD_SET_ENERGY`, and threads it into the dispatch. How the role value *combines* (replace vs. scale) is authored inside the pattern (e.g. wire `energy` through a `Math` multiply). Off by default; the `speed`/`palette` roles are follow-ups.
