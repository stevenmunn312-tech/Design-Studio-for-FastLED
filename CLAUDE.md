...
- 1-bit OLED panels render through a bus-agnostic `OledSurface` framebuffer (`src/state/oledSurface.ts`); each controller's own RAM quirks (e.g. the SH1106's column offset) live on its `OledController` descriptor rather than being fixed up per device. Layout screens (`src/state/infoDisplay.ts`) and the emitted C++ driver (`src/codegen/infoDisplayCpp.ts`) share the same geometry constants and glyph table, so the panel firmware draws exactly what the preview drew.
