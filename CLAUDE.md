...
- Transport semantics live once in `src/state/transportBridge.ts` — button debounce and rising edge, seek-as-change rather than value, runtime-to-display status — so a press cannot mean two things to `PlayerControls` and `TransportControl`.
- **Displays: registration points.** A new auxiliary display touches more registries than a normal node, and missing one fails quietly. Beyond the usual `NODE_LIBRARY`/evaluator/codegen/help/tests, register it in: `hardware.ts` (both hardware-owned sets), `partOptions.ts` (the exact module), `nodeLibrary.ts` `GPIO_PIN_PROPERTIES`, `busTopology.ts` `BUS_ASSIGNMENTS`, `hardwareManifest.ts` `collectPinUses`, `pinRetarget.ts` `PART_PIN_PLANS`, `cppGenerator.ts` `DISPLAY_TERMINAL_NODE_TYPES`, and `HardwarePane.tsx` `FIXTURE_PARTS`. Missing `PART_PIN_PLANS` costs twice over: the part keeps the pins of the board being left, and it never enters `claimed`, so parts that do retarget are handed its pins on top of it. `src/state/__tests__/hardwareRegistries.test.ts` holds the hand-written registries in step, so an omission fails there rather than on a bench. Prefer deriving a registry over adding a row to it — the Build Diagram's part list, its render table and the codegen walk roots are all derived now, each after being forgotten at least once. Controller quirks (an SH1106's column offset, a module's digit count) belong on a controller descriptor, never fixed up per device, and layout geometry plus glyph tables are generated into the emitted C++ from the shared modules rather than restated. A bus role also can't always be inferred from the property name alone — the same pin can be exclusive on one controller and a shareable bus line on another — so `collectPinUses` attaches a resolved `bus` to the pin use, which `findPinCollisions` prefers over the static `BUS_ASSIGNMENTS` lookup. See [auxiliary displays](docs/development/design/auxiliary-displays.md).
