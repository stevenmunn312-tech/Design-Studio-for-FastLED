...
- **`src/codegen/performanceGenerator.ts`** — a rules engine that maps a `SongAnalysis` to a timed `ShowFile`: a sorted event stream of `SET_PATTERN`/`SET_PALETTE`/`SET_SPEED`/`SET_BRIGHTNESS`/`BEAT_FLASH`/`TRANSITION` commands. Tuned by the node's `beatIntensity`/`energySensitivity`/`transitionDuration`/`patternHold`/`paletteMode` properties. **Within-section cycling:** rather than one pattern per section, each section is subdivided into `round(sectionDuration / patternHold)` slots (default `patternHold` 6 s), each slot switching to a *different* pattern with a same-section `TRANSITION` between — so a long section stays varied. Collapses to a single pattern when the section is short or fewer than two patterns are eligible for it (so short-section and single-pattern-collection shows are unchanged). Applies to both enum and collection modes; `SET_PALETTE`/`SET_SPEED`/`SET_ENERGY` stay section-level.
