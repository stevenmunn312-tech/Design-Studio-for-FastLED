...
- Colour TFT (ST7789/ST7789V) support exists as pure modules mirroring the OLED family — `src/state/tftSurface.ts` (surface primitives), `src/state/transportDisplay.ts` (Now Playing/Show Status layouts), `src/codegen/tftDisplayCpp.ts` (driver) — and `TransportDisplay` is now wired end to end: evaluator case in `graphEvaluator.ts` (`TftSurface` joined the `PortValue` union), emit case in `cppGenerator.ts`, the SD-player half in `playerSketchGenerator.ts`/`playerDisplays.ts` (`tft: PlayerTransportDisplay[]`), `tftControllerForProps` beside `oledControllerForProps` in `nodeLibrary.ts`, and validateGraph's prior refusal lifted — its RAM estimate now uses the driver's real `TFT_PANEL_RAM_BYTES` (~296 bytes) rather than a provisional full-frame 153,600. The node deliberately has no `artwork` input at all: the obvious wiring is an `image` port, which carries live `ImageData` capped at `IMAGE_MAX_DIM`, and bridging that to the baked RGB565 bytes the layout renders needs a scaler that would need a C++ twin — so both preview and firmware draw the same empty frame until the baker lands. `TransportDisplay` currently has inputs and no outputs, so both derived terminal registries (`SINK_NODE_TYPES`, `HOT_NODE_TYPES`) pick it up for free — the day it grows an output port it silently drops out of both at once and gets pruned from the sketch with nothing naming the port that caused it; see [auxiliary displays](docs/development/design/auxiliary-displays.md#displays-are-terminals) before adding one. `tftControllerFor` must match the longest controller name first (`ST7789V` starts with `ST7789`) or the 240x320 module silently gets the 240x240 descriptor. Descriptors state native-portrait geometry only; rotation is a node property, and `tftWindowOrigin` derives each rotation's RAM offset from one rule rather than a four-row table. Unlike the OLED, the panel has no device-side framebuffer (240x320 is 153 KB) — the browser surface keeps a real dirty bounding box, while the firmware instead caches each field's last-drawn text/value and repaints only what changed; both draw identical pixels, and that asymmetry is deliberate, not something to unify. The background paints once at setup rather than on the refresh deadline, since a full-panel fill would stall the LED loop. Baked artwork follows the same browser-bakes/device-only-blits rule as pattern thumbnails and for the same reason — no second scaler in C++ to disagree with the browser one. This driver needs `#include <SPI.h>` (`TFT_DISPLAY_CPP_INCLUDES`) in `cppGenerator.ts`, unlike the OLED's bit-banging; `playerSketchGenerator.ts` already had one for the SD card.
