...
- The LED preview is wall-clock driven. Do not reintroduce frame-count-dependent animation timing. Master Speed scales that one clock and is **accumulated, never multiplied** (`dt * speed`, not `t * speed`, which would jump every animation the instant the knob moved): the browser slides its clock's origin — the same mechanism the pause uses — and `masterSpeedCpp.ts` keeps a `_tAnim` accumulator in place of `millis() / 1000`. Both read the *previous* frame's speed, because a control computed from scaled time cannot be turned back up out of a freeze. LED-output Enabled/Brightness (`state/ledOutputRuntime.ts`) are per output and applied in the evaluator's `MatrixOutput` case, so preview, per-output previews, recordings and the live stream cannot disagree; the firmware half lands after the blit, the one point every geometry branch has converged on the physical array. A music player refuses Master Speed on purpose — its animation time *is* the track position.
