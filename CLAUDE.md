...
**Group-input modulation (slice 2, `energy` + `speed` roles).** The `PerformanceGenerator.useGroupInputs` toggle drives each collected pattern's exposed `GroupInput` "roles" from the analysis. A pattern opts in by exposing a `GroupInput` whose `paramId` is a role name — `energy` or `speed` (rename it from the default `param0` in the node's property editor). The generator emits a `SET_ENERGY` event (0–1 section energy) per section; the **`speed` role reuses the existing `SET_SPEED`** event (no new command), normalising its 0–2 multiplier to 0–1 (`min(1, speed/2)` in preview; `constrain(params[0]*0.5f, 0, 1)` in firmware, which still also sets the global `animSpeed`). **Preview:** `renderShowFrame(show, ms, W, H, groups, useGroupInputs)` evaluates the pattern group's subgraph directly (its `GroupOutput` terminal) and, when on, passes `{ energy, speed }` through the evaluator's `groupInputs` path. **Codegen:** with `useGroupInputs`, `buildPattern` keeps role-tagged `GroupInput` nodes (instead of stripping them) and `generateCpp` emits them as `float n_<id>_out = <paramId>;`; `buildPatternRenderers(ids, groups, roleParams)` widens each `render_pN` to `(uint32_t ms, float energy, float speed)` and reports `params`; `generatePlayerSketch` declares the role globals, handles their commands, and threads them into the dispatch (`buildShowPayload` passes `['energy','speed']` when the toggle is on). How the role value *combines* (replace vs. scale) is authored inside the pattern (e.g. wire the role through a `Math` multiply). Off by default; the `palette` role is a follow-up.
