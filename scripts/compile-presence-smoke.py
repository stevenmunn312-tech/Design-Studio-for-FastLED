"""Compile a generated presence-sensor fixture through the helper's real build path; never flash."""
import argparse
...
with log_path.open("w", encoding="utf-8") as log:
    ...
    if args.engine == "fbuild":
        result = drain(helper._compile_upload_fbuild("Presence sensor smoke", ino, args.fqbn, "", 16), log)
        ...
        with helper._sketch_workspace(f"presence_smoke_{board}", ino) as workspace:
            result = drain(helper._compile_upload("Presence sensor smoke", workspace, args.fqbn, ""), log)
